...
function Product() {
    ...
    const product = data.filter(product => product.productCode === params.productCode)[0];
    ...
    return (
        <div class="flex flex-col bg-orange-100 min-h-screen">
            ...
            <main class="md:mt-8 md:ml-64 md:w-1/2 flex flex-row">
                ...
                <img class="object-contain rounded-lg ml-8" src={product.productImage} alt={`${product.name} in an interesting scenery`}/>
            </main>
        </div>
    );
}
